...
type IntegrationEventType = "new_lead" | "status_change";
...
function validateWebhookRequest(data: unknown): {
  valid: boolean;
  errors: string[];
  sanitized?: WebhookRequest;
} {
  ...
  const validation = validateFields([
    {
      result: validateEnum(req.event, "Event", ["new_lead", "status_change"] as const, true),
      field: "event",
    },
  ...
  return {
    ...
    errors: [],
    sanitized: {
      event: validation.sanitized.event as IntegrationEventType,
      lead: sanitizedLead,
...
function configSupportsEvent(config: Record<string, unknown> | null | undefined, event: IntegrationEventType) {
...
serve(async (req) => {
  ...
  try {
    ...
    const supportedIntegrations = (integrations || []).filter((integration) =>
      configSupportsEvent(integration.config as Record<string, unknown> | undefined, event)
    );

    console.log(`Found ${supportedIntegrations.length} active integrations to trigger for ${event}`);
    ...
    for (const integration of supportedIntegrations) {
      ...
      try {
        ...
        console.log(
          `Webhook ${integration.service_name} responded with status: ${response.status}`
        );
      } catch (err) {
